...
export class ActivityAreaView {
    ...
    private getConnectedUsers(): Observable<boolean> {
        return this.userService.loadConnectedUsers().pipe(
            map((connectedUsers) => {
                ...
                this.activityAreaPage.lines.forEach((line) => {
                    line.connectedUsersText = this.convertConnectedUsersToString(line.connectedUsers);
                });
                return true;
            })
        );
    ...
    private convertConnectedUsersToString(connectedUsers: string[]): string {
        let connectedUsersText = '';
        connectedUsers.forEach((connectedUser) => {
            if (connectedUsersText.length > 0) connectedUsersText += ', ';
            connectedUsersText += connectedUser;
        });
        return connectedUsersText;
    }
    ...
    public stopUpdatedRegularyConnectedUser() {
